...
void calculate_Jacobian_H(double xR, double yR, double xL, double yL, double out[][4], int &n)
{
    ...
    out[0][0] = (xL - xR) / hypot(xL - xR,yL - yR);
    out[0][1] = (yL - yR) / hypot(xL - xR,yL - yR);
    out[1][0] = (yR - yL)/((xL - xR)*(xL - xR) + (yL - yR)*(yL - yR));
    out[1][1] = (xR - xL)/((xL - xR)*(xL - xR) + (yL - yR)*(yL - yR));
    out[2][2] = 1.0;
    out[3][3] = 1.0;
}
...
{
    n=2;
    out[0] = r * cos(theta + hS) + xS;
    out[1] = r * sin(theta + hS) + yS;
}
...
{
    n=2;
    memset(out,0, 2*2*sizeof(double));
    out[0][0] = sigma_r*sigma_r;
    out[1][1] = sigma_theta*sigma_theta;
}
...
void Jacobian_of_velocity_f(double dt, double out[][4], int &n, int &m)
{
    m=2;
    n=4;
    memset(out, 0, 2*4*sizeof(double));
    out[0][0] = -1/dt;
    out[0][1] = 1/dt;
    out[1][2] = -1/dt;
    out[1][3] = 1/dt;
...
{
    n=2;
    out[0][0] = cos(theta + rH);
    out[0][1] = -r*sin(theta + rH);
    out[1][0] = sin(theta + rH);
    out[1][1] = r*cos(theta + rH);
}
...
{
    double out1[2][2], out2[2][2];
    int n1, n2;

    calculate_Jacobian_of_inv_h(r, theta, rH, out1, n1);
    traspuesta(out1, 2, out2, n2);
    matrizMatriz(out1, 2, 2, out2, 2, 2, out, n, n2);
    for(int i = 0; i < 2; i++)
        for(int j = 0; j < 2; j++)
            out[i][j] = R*out[i][j];
...
void matrizMatriz(double m1[][4], int af, int ac, double m2[][4], int bf, int bc, double out[][4], int &nf, int &nc)
{
    double sum=0;
    int i, j, k;
...
{
    double out[2][4], temp[4][4], temp1[4][4], temp2[4][4],  prod1[4][4], prod2[4][4];
    int nf, nc;

    memset(temp, 0, 4*4*sizeof(double));
    temp[0][0] = P_landmark[0][0];
    temp[0][1] = P_landmark[0][1];
    temp[1][0] = P_landmark[1][0];
    temp[1][1] = P_landmark[1][1];
    temp[2][2] = R_c[0][0];
    temp[2][3] = R_c[0][1];
    temp[3][2] = R_c[1][0];
    temp[3][3] = R_c[1][1];
    memset(temp1, 0, 4*4*sizeof(double));
    memset(temp2, 0, 4*4*sizeof(double));
    Jacobian_of_velocity_f(dt, out, nf, nc);
    for(int i=0; i < 2; i++)
        for(int j=0; j < 4; j++)
        {
            temp1[i][j] = out[i][j];
    ...
    matrizMatriz(temp1, 4, 4, temp, 4, 4, prod1, nf, nc);
    matrizMatriz(prod1, 4, 4, temp2, 4, 4, prod2, nf, nc);
    for(int i=0; i < 2; i++)
        for(int j=0; j < 2; j++)
            outt[i][j] = prod2[i][j];
}
...
{
    //
    extern double sigma_r, sigma_theta, upsilon_x, upsilon_y, upsilon_vx, upsilon_vy, Q[4][4], U[4][4];

    //For sigma_r: For this simulation the value of sigma_r is kind of
    //arbitrary. Let's just say 5m for simplicity.
    sigma_r = 5; //Sensor range standard deviation in m
    //For sigma_theta: Also kind of arbitrary, so we'll say 2 degrees
    sigma_theta = 2*M_PI/180; //Sensor bearing standard deviation in radians

    memset(U, 0, 4*4*sizeof(double));
    ...
    Q[0][0] = upsilon_x*upsilon_x;
    Q[1][1] = upsilon_y*upsilon_y;
    Q[2][2] = upsilon_vx*upsilon_vx;
    Q[3][3] = upsilon_vy*upsilon_vy;
