...
#include <iostream>
#include "GM_PHD_Update.h"
#include "GM_PHD_Initialisation.h"

using namespace std;

void zeros(int n, int m, double out[][NUM]);

void GM_PHD_Update(void) {
    ...
    extern double w_k[NUM], m_k[NUM_F][NUM], P_k[NUM_F][NUM], Z[2][53], Pk_k_minus_1[NUM_F][NUM], prob_detection,
            ...
            S[4][NUM], K[4][NUM], P_k_k[4][NUM], simTarget1State[4];
    extern int numTargets_Jk_k_minus_1, P_range[4], VERBOSE, weightDataRange[2], numTargets_Jk;

    //New variables
    extern double newP[4][4], thisZ[4], prevX[2], thisV[2], delta[4], m_new[4], P_new[4][4], thisEta[4], weight_tally,
            measZ[2], thisPos[4];
    extern int L, thisIndex, old_P_range[4], new_P_range[4], S_range[4], w_new, K_range[4], old_weight, new_weight, thisW;
    ...
    zeros(1, numTargets_Jk_k_minus_1 * 53 + numTargets_Jk_k_minus_1, out);
    for (int i = 0; i < numTargets_Jk_k_minus_1 * 53 + numTargets_Jk_k_minus_1; i++) {
        w_k[i] = out[0][i];
    }
    zeros(4, numTargets_Jk_k_minus_1 * 53 + numTargets_Jk_k_minus_1, m_k);
    zeros(4, 4 * (numTargets_Jk_k_minus_1 * 53 + numTargets_Jk_k_minus_1), P_k);
