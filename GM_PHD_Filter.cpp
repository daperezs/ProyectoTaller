...
#include "GM_PHD_Create_Birth.h"
#include "GM_EKF_PHD_Create_Birth.h"
#include "GM_PHD_Calculate_Performance_Metric.h"
#include "GM_PHD_Simulate_Plot.h"
#include "GM_EKF_PHD_Simulate_Plot.h"

using namespace std;

int main()
{
    double t1start[NUM], t2start[NUM], P_birth[4][8];
    string s;
    ...
    if(KNOWN_TARGET == 1)
    {
        ...
        for(int i = 0; i < 2; i++)
        {
            m_birth[i][0] = simTarget1Start[i];
            m_birth[i][1] = simTarget2Start[i];
            m_birth[2 + i][0] = simTarget1Vel[i];
            m_birth[2 + i][1] = simTarget2Vel[i];
        }
        ...
        for(int i = 0; i < 4; i++)
        {
            for(int j = 0; j < 4; j++)
            {
                P_birth[i][j] = covariance_birth[i][j];
                P_birth[i][j + 4] = covariance_birth[i][j];
            }
        }
        numBirthedTargets = 2;
    }
    ...
    while (k < endTime)//k = timestep
    {
        ...
        else
        {
            ...
            //Step 2: Prediction for existing targets                                                                                                                            %Step 2: Prediction for existing targets
